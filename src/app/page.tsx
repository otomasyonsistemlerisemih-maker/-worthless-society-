  ...
  return (
    <>
      ...
      <div id="preloader" className={!showPreloader ? 'hide' : ''} style={{ display: showPreloader ? 'flex' : 'none' }}>
        ...
        {/* Sound Toggle Button */}
        <button
          className="preloader-sound-btn"
          aria-label={videoMuted ? 'Sesi Aç' : 'Sesi Kapat'}
          onClick={() => {
            const newMuted = !videoMuted;
            setVideoMuted(newMuted);
            if (preloaderVideoRef.current) {
              preloaderVideoRef.current.muted = newMuted;
              if (!newMuted) {
                preloaderVideoRef.current.volume = 0.25;
                preloaderVideoRef.current.play();
              }
            }
          }}
        >
          ...
          <span>{videoMuted ? 'SES' : 'SUSTUR'}</span>
        </button>
